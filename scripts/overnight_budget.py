...
def submit(path, script, name, seconds, node=None):
    ...
    with locked(path) as state:
        refresh(state)
        for lane in ("A", "B"):
            if name.startswith(f"dfnight-{lane}") and state.get(
                f"{lane}_lane_status", ""
            ).startswith("PAUSED"):
                raise BudgetStop(f"{lane}_LANE_PAUSED_AFTER_FINITE_REVISIONS")
...
def claim_call(lane, group, seed):
    ...
    with locked(path) as state:
        refresh(state)
        if state.get(f"{lane}_lane_status", "").startswith("PAUSED"):
            raise BudgetStop(f"{lane}_LANE_PAUSED_AFTER_FINITE_REVISIONS")
        if utc() >= cutoff(state):
